...
interface Role {
  ...
  isSystem: boolean;
  userCount: number;
  permissions: Array<{ menuCode: string; permissionType: 'VIEW' | 'MODIFY' }>;
}
...
export default function RoleManagementPage() {
  ...
  const handleEdit = async (role: Role) => {
    try {
      ...
      setFormData({ name: response.data.name, description: response.data.description, permissions: response.data.permissions || [] });
      setIsModalOpen(true);
          ...
          <div key={role.id} className="bg-white rounded-lg shadow p-6">
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-xs bg-green-100 text-green-600 px-2 py-1 rounded">
                {role.permissions?.filter(p => p.permissionType === 'MODIFY').length || 0} modificar
              </span>
            </div>
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className="w-full px-3 py-2 border rounded-md" rows={2} required />
                </div>
                <div>
                      ...
                      <tbody>
                        {MENU_CODES.map((menu) => (
                          <tr key={menu.code} className="border-b">
                            ...
                            <td className="text-center"><input type="checkbox" checked={hasPermission(menu.code, 'VIEW')} onChange={() => togglePermission(menu.code, 'VIEW')} className="w-4 h-4" /></td>
                            <td className="text-center"><input type="checkbox" checked={hasPermission(menu.code, 'MODIFY')} onChange={() => togglePermission(menu.code, 'MODIFY')} className="w-4 h-4" /></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
