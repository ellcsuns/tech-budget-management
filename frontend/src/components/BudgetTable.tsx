...
// BudgetTable component - test write
const MONTHS = ['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic'];

export default function BudgetTable(props: BudgetTableProps) {
  const { expenses, editedCells, validationErrors, canEdit, onCellEdit, onRemoveRow } = props;
  ...
  const isCellEdited = (expenseId: string, month: number): boolean =>
    editedCells.has(getCellKey(expenseId, month));

  const getCellError = (expenseId: string, month: number): string | undefined =>
    validationErrors.get(getCellKey(expenseId, month));
  ...
  const grandTotal = useMemo(() =>
    expenseTotals.reduce((s, t) => s + t.total, 0), [expenseTotals]);
        ...
        <thead className="bg-gray-50 sticky top-0">
          <tr>
            ...
            <th
              ...
              style={{ width: descWidth, minWidth: 100 }}
            >
              Descripción
              <span
            ...
            return (
              <tr key={expense.id} className="hover:bg-gray-50 group">
                ...
                <td
                  ...
                  style={{ width: descWidth, maxWidth: descWidth, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
                >
                  {expense.description}
                </td>
        ...
        <tfoot className="bg-gray-100 font-bold">
          <tr>
            ...
            {monthlyTotals.map((t, i) => (
              <td key={i} className="px-4 py-3 text-sm text-right border border-gray-200">
                {fmt(t)}
              </td>
            ))}
