...
export default function FilterPanel({ budgetLines, filters, onFiltersChange, showBaseToggle, showBase, onShowBaseChange, showSummaryToggle, showSummary, onShowSummaryChange }: FilterPanelProps) {
  ...
  return (
    <div className="flex items-center gap-1.5 flex-wrap mb-4">
      {/* Description */}
      <DropBtn
        label={t('table.description') || 'Descripción'}
      ...
      >
        ...
        <div className="max-h-[200px] overflow-y-auto">
          {descriptions.sort().map(desc => (
            <ChkItem key={desc} label={desc} checked={searchTerms.length === 0 || searchTerms.some(term => desc.toLowerCase().includes(term.toLowerCase()))} onChange={() => toggleDescription(desc)} />
          ))}
        </div>
      </DropBtn>
      ...
      {/* Values */}
      <DropBtn
        label={t('filter.values') || 'Valores'}
        icon={<HiOutlineTableCells className="w-3.5 h-3.5" />}
        active={[filters.visibleColumns.budget, filters.visibleColumns.committed, filters.visibleColumns.real, filters.visibleColumns.diff].some(v => !v)}
      >
      ...
      {currencies.length > 0 && (
        <DropBtn
          label={t('label.currency') || 'Moneda'}
          icon={<HiOutlineCurrencyDollar className="w-3.5 h-3.5" />}
          active={!!filters.currencies}
          onClear={() => onFiltersChange({ ...filters, currencies: undefined })}
        >
          {currencies.map(c => (
            <ChkItem key={c} label={c} checked={!filters.currencies || filters.currencies.includes(c)} onChange={() => toggleCurrency(c)} />
          ))}
        </DropBtn>
      )}
      ...
      {financialCompanies.length > 0 && (
        <DropBtn
          ...
          icon={<HiOutlineBuildingOffice2 className="w-3.5 h-3.5" />}
        ...
        >
          {financialCompanies.map((c: FinancialCompany) => (
            <ChkItem key={c.id} label={`${c.code} — ${c.name}`} checked={!filters.financialCompanyIds || filters.financialCompanyIds.includes(c.id)} onChange={() => toggleFinancialCompany(c.id)} />
          ))}
        </DropBtn>
      )}
      ...
      {expenseCategories.length > 0 && (
        <DropBtn
          ...
          icon={<HiOutlineTag className="w-3.5 h-3.5" />}
          active={!!filters.categories}
          onClear={() => onFiltersChange({ ...filters, categories: undefined })}
        >
          {expenseCategories.map((c: ExpenseCategory) => (
            <ChkItem key={c.id} label={c.name} checked={!filters.categories || filters.categories.includes(c.id)} onChange={() => toggleCategory(c.id)} />
          ))}
        </DropBtn>
      )}
      ...
      {showSummaryToggle && onShowSummaryChange && (
        <button onClick={() => onShowSummaryChange(!showSummary)}
          ...
          <HiOutlineTableCells className="w-3.5 h-3.5" />
      ...
      {/* Clear all */}
      {hasActiveFilters && (
        <button onClick={clearFilters} className="inline-flex items-center gap-1 px-2 py-1.5 text-xs text-accent hover:opacity-70 transition-opacity" title={t('filter.clearFilters')}>
